---
layout: default
title: User Guides
---

# User Guides

Comprehensive step-by-step guides for using the Financial Planner application.

## 📚 Guide Categories

### Getting Started Guides
- [Portfolio Data Entry Guide](user/PORTFOLIO_DATA_ENTRY_GUIDE.md)
- [Portfolio Analytics Guide](user/PORTFOLIO_ANALYTICS_GUIDE.md)
- [Portfolio Rebalancing Guide](user/PORTFOLIO_REBALANCING_GUIDE.md)
- [Brokerage Connections Guide](guides/brokerage-connections.md) ⭐ NEW
- [Market Trend Analysis Guide](user/MARKET_TREND_GUIDE.md) — regime labels, sub-phases, and tactical guidance
- [Withdrawal Strategies Guide](user/STRATEGY_README.md)
- [Social Security Guide](implementation/SSI_INTEGRATION_GUIDE.md)

### Advanced Guides
- [BETR Roth Conversion](advanced/betr-guide.md)
- [Mega Backdoor Roth Strategy](advanced/mega-backdoor-roth.md)
- [Bucket Strategy Implementation](advanced/bucket-strategy.md)
- [Monte Carlo Analysis](advanced/monte-carlo.md)

### Technical Guides
- [Configuration System](technical/configuration.md)
- [Data File Formats](technical/data-formats.md)
- [Testing Guide](technical/testing.md)
- [Performance Optimization](technical/performance.md)

## Quick Reference Guides

### Brokerage Connections (NEW - March 2026)

**Essential Tasks:**
1. [Setting Up SnapTrade](guides/brokerage-connections.md#setup)
2. [Connecting Your First Account](guides/brokerage-connections.md#connecting)
3. [Syncing Portfolio Data](guides/brokerage-connections.md#syncing)
4. [Managing Connections](guides/brokerage-connections.md#managing)

**Key Features:**
- Automatic portfolio synchronization
- OAuth 2.0 secure authentication
- Support for 12,000+ institutions
- Encrypted credential storage
- One-click account updates

**Quick Links:**
- [SnapTrade Quick Start Guide](user/SNAPTRADE_QUICKSTART.md)
- [Implementation Summary](implementation/SNAPTRADE_IMPLEMENTATION_SUMMARY.md)
- [Integration Plan](implementation/SNAPTRADE_INTEGRATION_PLAN.md)

### Portfolio Management

**Essential Tasks:**
1. [Adding a New Account](user/PORTFOLIO_DATA_ENTRY_GUIDE.md#first-time-setup)
2. [Updating Account Balances](user/PORTFOLIO_DATA_ENTRY_GUIDE.md#monthly-update-workflow)
3. [Connecting Brokerage Accounts](guides/brokerage-connections.md) ⭐ NEW
4. [Rebalancing Your Portfolio](user/PORTFOLIO_REBALANCING_GUIDE.md)
5. [Tracking Performance](user/PORTFOLIO_ANALYTICS_GUIDE.md)

**Key Features:**
- Interactive holdings editor
- Automatic brokerage sync ⭐ NEW
- Real-time validation
- Performance analytics
- Tax-efficient rebalancing

### Tax Optimization

**Core Strategies:**
1. [Roth Conversions](guides/tax-optimization.md#roth-conversions)
2. [Tax-Loss Harvesting](guides/tax-optimization.md#tax-loss-harvesting)
3. [Capital Gains Management](guides/tax-optimization.md#capital-gains)
4. [IRMAA Avoidance](guides/tax-optimization.md#irmaa)

**Advanced Topics:**
- BETR algorithm usage
- Mega Backdoor Roth
- QCD strategies
- State tax optimization

### Withdrawal Strategies

**Planning Phases:**
1. [Pre-Retirement Accumulation](guides/withdrawal-strategies.md#accumulation)
2. [Early Retirement (Pre-59½)](guides/withdrawal-strategies.md#early-retirement)
3. [Pre-Social Security (59½-62)](guides/withdrawal-strategies.md#pre-ss)
4. [Social Security Bridge (62-70)](guides/withdrawal-strategies.md#ss-bridge)
5. [RMD Management (70+)](guides/withdrawal-strategies.md#rmd-phase)
6. [Estate Planning](guides/withdrawal-strategies.md#estate)

**Key Concepts:**
- Withdrawal sequencing
- Tax bracket management
- Account prioritization
- Dynamic optimization

### Healthcare Planning

**Coverage Options:**
1. [Long-Term Care Planning](guides/healthcare-planning.md#ltc)
2. [HSA Optimization](guides/healthcare-planning.md#hsa)
3. [Medicare Enrollment](guides/healthcare-planning.md#medicare)
4. [ACA Marketplace](guides/healthcare-planning.md#aca)

**Cost Management:**
- Premium calculations
- IRMAA threshold planning
- HSA contribution strategies
- LTC insurance vs. self-funding

### Social Security

**Claiming Strategies:**
1. [Optimal Claiming Age](guides/social-security.md#claiming-age)
2. [Spousal Benefits](guides/social-security.md#spousal)
3. [Survivor Benefits](guides/social-security.md#survivor)
4. [Tax Implications](guides/social-security.md#taxes)

**Coordination:**
- File and suspend
- Restricted application
- Earnings test considerations
- Windfall provisions

## Step-by-Step Tutorials

### Tutorial 0: Connecting Your Brokerage Accounts (NEW)

**Duration:** 10 minutes

1. **Sign Up for SnapTrade**
   - Visit [snaptrade.com](https://snaptrade.com)
   - Create account (free tier: 5 connections)
   - Get API credentials

2. **Configure Application**
   - Generate encryption key
   - Add credentials to `.env` file
   - Restart application

3. **Connect First Account**
   - Navigate to Portfolio Hub → Connections
   - Click "Connect Account"
   - Complete OAuth authentication
   - Verify connection

4. **Sync Portfolio Data**
   - Click "Sync Now"
   - Review synced holdings
   - Merge with existing portfolio
   - Verify data accuracy

**See:** [Complete SnapTrade Setup Guide](user/SNAPTRADE_QUICKSTART.md)

### Tutorial 1: Setting Up Your First Plan

**Duration:** 15 minutes

1. **Launch the Application**
   ```bash
   ./run.sh
   ```

2. **Configure Personal Information**
   - Navigate to Configuration page
   - Enter names and birth dates
   - Set retirement ages
   - Save configuration

3. **Add Portfolio Accounts**
   - Go to Portfolio Hub
   - Click "Add Account"
   - Enter account details
   - Save changes

4. **Review Dashboard**
   - Check net worth summary
   - Review asset allocation
   - Verify projections

5. **Run Initial Analysis**
   - Navigate to Strategy tab
   - Review withdrawal plan
   - Check tax projections

### Tutorial 2: Optimizing Roth Conversions

**Duration:** 20 minutes

1. **Understand Current Tax Situation**
   - Review current income
   - Check tax bracket
   - Identify conversion capacity

2. **Run BETR Analysis**
   - Navigate to Advanced Strategies
   - Select BETR Roth Conversion
   - Review recommendations

3. **Evaluate Conversion Scenarios**
   - Compare different amounts
   - Check IRMAA impact
   - Review long-term benefits

4. **Implement Strategy**
   - Update portfolio projections
   - Schedule conversions
   - Monitor tax brackets

### Tutorial 3: Creating a Withdrawal Strategy

**Duration:** 25 minutes

1. **Define Retirement Goals**
   - Set retirement date
   - Determine income needs
   - Identify expense categories

2. **Configure Strategy Parameters**
   - Navigate to Strategy tab
   - Set withdrawal preferences
   - Choose optimization goals

3. **Review 6-Stage Plan**
   - Examine each life stage
   - Understand account sequencing
   - Check tax implications

4. **Run Monte Carlo Simulation**
   - Set simulation parameters
   - Review success probability
   - Analyze risk factors

5. **Refine and Optimize**
   - Adjust spending levels
   - Modify asset allocation
   - Test sensitivity

### Tutorial 4: Portfolio Rebalancing

**Duration:** 15 minutes

1. **Review Current Allocation**
   - Check Portfolio Hub Overview
   - Compare to target allocation
   - Identify drift

2. **Generate Rebalancing Plan**
   - Navigate to Optimization tab
   - Review recommendations
   - Check tax impact

3. **Evaluate Tax Efficiency**
   - Identify tax-loss harvesting
   - Check wash sale rules
   - Optimize transaction order

4. **Implement Changes**
   - Execute recommended trades
   - Update portfolio data
   - Verify new allocation

## Common Workflows

### Monthly Review Workflow

1. **Update Account Balances**
   - Import latest statements
   - Update market values
   - Record contributions

2. **Review Performance**
   - Check returns vs. benchmarks
   - Analyze risk metrics
   - Identify trends

3. **Check Rebalancing Needs**
   - Review allocation drift
   - Evaluate rebalancing triggers
   - Plan tax-efficient trades

4. **Update Projections**
   - Refresh retirement timeline
   - Adjust expense estimates
   - Review goal progress

### Annual Planning Workflow

1. **Year-End Tax Planning**
   - Review tax-loss harvesting
   - Plan Roth conversions
   - Optimize charitable giving
   - Check RMD requirements

2. **Contribution Planning**
   - Maximize 401(k) contributions
   - Fund IRA accounts
   - HSA contributions
   - 529 plan funding

3. **Strategy Review**
   - Update retirement timeline
   - Adjust withdrawal strategy
   - Review Social Security plan
   - Update estate plan

4. **Performance Analysis**
   - Annual return calculation
   - Risk assessment
   - Benchmark comparison
   - Goal tracking

### Pre-Retirement Workflow

1. **Retirement Readiness Check**
   - Calculate retirement number
   - Review income sources
   - Verify healthcare coverage
   - Check Social Security estimates

2. **Transition Planning**
   - Finalize withdrawal strategy
   - Set up income streams
   - Optimize tax situation
   - Review insurance needs

3. **Account Consolidation**
   - Roll over old 401(k)s
   - Consolidate IRAs
   - Simplify account structure
   - Update beneficiaries

4. **Final Optimization**
   - Last-minute Roth conversions
   - Tax-loss harvesting
   - Asset allocation adjustment
   - Emergency fund verification

## Best Practices

### Data Management
- Update balances monthly
- Back up configuration regularly
- Maintain historical records
- Document assumptions

### Tax Planning
- Review quarterly
- Plan conversions annually
- Monitor bracket thresholds
- Coordinate with tax advisor

### Portfolio Management
- Rebalance systematically
- Harvest losses opportunistically
- Minimize transaction costs
- Consider tax implications

### Strategy Execution
- Start planning early
- Review assumptions regularly
- Adjust for life changes
- Stay disciplined

## Video Tutorials (Coming Soon)

- Getting Started Walkthrough
- Portfolio Setup Guide
- BETR Conversion Tutorial
- Monte Carlo Analysis
- Rebalancing Strategies

## Interactive Examples

Explore pre-configured scenarios:
- [Standard Retirement](../example1_strategy.csv)
- [Early Retirement (FIRE)](../example2_early_retire.csv)
- [High Income Optimization](../example3_high_income.csv)
- [Custom Strategy Template](../example4_custom.csv)

## Additional Resources

### Documentation
- [Complete Solution Guide](implementation/COMPLETE_SOLUTION_GUIDE.md)
- [Configuration Guide](user/CONFIG_GUIDE.md)
- [SnapTrade Quick Start](user/SNAPTRADE_QUICKSTART.md) ⭐ NEW
- [SnapTrade Implementation Summary](implementation/SNAPTRADE_IMPLEMENTATION_SUMMARY.md) ⭐ NEW
- [Income & Expense Guide](user/INCOME_EXPENSE_GUIDE.md)
- [Portfolio Data Entry Guide](user/PORTFOLIO_DATA_ENTRY_GUIDE.md)

### Implementation Guides
- [Market Trend Analysis Guide](user/MARKET_TREND_GUIDE.md) — Bull / Neutral / Bear regimes and Accumulation / Consolidating / Distribution sub-phases
- [Bucket Strategy Guide](user/BUCKET_STRATEGY_GUIDE.md)
- [HSA Integration Guide](implementation/HSA_INTEGRATION_GUIDE.md)
- [LTC Planning Guide](implementation/LTC_PLANNING_GUIDE.md)
- [Single Person Mode Guide](user/SINGLE_PERSON_MODE_GUIDE.md)

### Technical Documentation
- [API Reference](api-reference.md)
- [Module Documentation](technical/modules.md)
- [Testing Guide](technical/testing.md)
- [Performance Guide](technical/performance.md)

## Getting Help

- Check [Troubleshooting](../README.md#troubleshooting)
- Review [Known Issues](../README.md#known-issues--limitations)
- Visit [GitHub Issues](https://github.com/yourusername/retirement_planning/issues)
- Join community discussions

---

[← Back to Home](index.md) | [Next: API Reference →](api-reference.md)
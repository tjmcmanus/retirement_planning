        ...
        **kwargs: Any
        ...
        if _jan_spending_withdrawal > 0:
            ...
            if _jan_trad_to_cash > 0:
                ...
                logger.info(
                    f"Year {year} Stage 5 [Phase 1 - Spending]: "
...
# Made with Bob
    ...
    def _calculate_anticipated_buffer_needs(
        ...
        try:
            from strategy import calculate_anticipated_buffer_needs
    ...
    def _plan_january_bracket_fill_withdrawal(
        ...
        if not use_january_strategy:
            logger.debug("Stage 5: January Bracket-Fill Strategy not enabled in config")
            return None
        ...
        try:
            ...
            logger.warning(f"Stage 5: Could not read bracket data: {e}")
            return None
        
        if not bracket_12_upper or not std_deduction_value:
            logger.warning("Stage 5: Missing bracket or deduction data for January strategy")
            return None
        ...
        # Plan the withdrawal
        try:
            # Withholding rate = stage 5 max conversion rate (from config) ÷ 100
            try:
                from config import get_config_value
                _stage_rate_pct = float(get_config_value(
                    'tax_strategy', 'stage_5_max_conversion_rate', 12
                ))
            except:
            ...
            logger.info(
                f"Stage 5: January Bracket-Fill plan generated: "
            ...
            logger.warning(f"Stage 5: January Bracket-Fill planning failed: {e}")
            return None

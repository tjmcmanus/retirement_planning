import streamlit as st
from datetime import datetime
import pandas as pd
import numpy as np
import plotly.express as px
import plotly.graph_objects as go
import logging
from streamlit_card import card
from streamlit_extras.metric_cards import style_metric_cards
from streamlit_extras.add_vertical_space import add_vertical_space
from load_data import get_month_account_values,get_cap_gains_brackets, get_income_tax_brackets, get_net_worth, get_medicare_costs, get_atm_costs, get_std_deduction, get_networth_by_month
from calculations import calc_roth_conversions_tax, getlower_atm_amount_n_deduction,calc_roth_conversions,calc_agi,calc_daf_value,getUpperIncomeRate,calculate_atm, calculate_std_deduction,get_std_deduction_by_year, calculate_irmma_penalty, calculate_cap_gains, calculate_taxable_income
from portfolio import get_portfolio_dividend_total,get_current_dividend,get_current_price,get_entry_in_portfolio,get_list_of_tickers,get_purchase_price,get_qty,getPortfolioData,calculate_cost_basis,calculate_current_value, get_ticker_name,get_sector,color_negative_positive,build_portfolio_display
from income_expense import build_income_expenses_display,calculate_taxes
from planning_app import clear_submit

st.set_page_config(page_title="Caclculators", page_icon="😊", layout="wide")

# Configure logging
import os
log_level = logging.getLevelName(os.getenv('LOG_LEVEL', 'WARNING'))
logging.basicConfig(
    level=log_level,
    format='%(asctime)s - %(levelname)s - %(funcName)s:%(lineno)d - %(message)s',
    datefmt='%Y-%m-%d %H:%M:%S'
)
logger = logging.getLogger(__name__)

# Load net worth data with exception handling using current month/year
try:
    logger.debug("Loading net worth data")
    current_date = datetime.now()
    current_month = current_date.month
    current_year = current_date.year
    
    # Get current and previous month's data
    detailed_df, summary_df = get_networth_by_month(current_month, current_year)
    
    # Get previous month for comparison
    prev_month = current_month - 1 if current_month > 1 else 12
    prev_year = current_year if current_month > 1 else current_year - 1
    prev_detailed_df, prev_summary_df = get_networth_by_month(prev_month, prev_year)
    
    # Create a networth dataframe structure compatible with existing code
    # Extract values from summary dataframes
    networth = pd.DataFrame({
        'cash': [
            prev_summary_df[prev_summary_df['account_type'] == 'Cash']['market_value'].sum() if not prev_summary_df.empty else 0,
            summary_df[summary_df['account_type'] == 'Cash']['market_value'].sum() if not summary_df.empty else 0
        ],
        'taxable': [
            prev_summary_df[prev_summary_df['account_type'] == 'Brokerage']['market_value'].sum() if not prev_summary_df.empty else 0,
            summary_df[summary_df['account_type'] == 'Brokerage']['market_value'].sum() if not summary_df.empty else 0
        ],
        'tax_free': [
            prev_summary_df[prev_summary_df['account_type'] == 'Roth']['market_value'].sum() if not prev_summary_df.empty else 0,
            summary_df[summary_df['account_type'] == 'Roth']['market_value'].sum() if not summary_df.empty else 0
        ],
        'tax_deferred': [
            prev_summary_df[prev_summary_df['account_type'] == 'Traditional']['market_value'].sum() if not prev_summary_df.empty else 0,
            summary_df[summary_df['account_type'] == 'Traditional']['market_value'].sum() if not summary_df.empty else 0
        ]
    })
    
    # Calculate total
    networth['total'] = networth['cash'] + networth['taxable'] + networth['tax_free'] + networth['tax_deferred']
    
    logger.debug(f"Net worth data loaded successfully for {current_month}/{current_year}")
    logger.debug(f"Current total net worth: ${networth['total'].iloc[-1]:,.2f}")
    
except Exception as e:
    logger.error(f"Failed to load net worth data: {str(e)}", exc_info=True)
    st.error(f"Error loading net worth data: {str(e)}")
    # Create empty dataframe with expected structure
    networth = pd.DataFrame({
        'cash': [0, 0],
        'taxable': [0, 0],
        'tax_free': [0, 0],
        'tax_deferred': [0, 0],
        'total': [0, 0]
    })
conversion_tab, something_tab = st.tabs(["Tax planner","something"])
with conversion_tab:
    st.header("Withdrawl and Conversion calculator")
   # add_vertical_space(2)
    
    # Display net worth metrics with exception handling
    try:
        logger.debug("Rendering net worth metrics")
        col1row3, col1row4, col1row5, col1row6,col1row7 = st.columns(5)
        
        with col1row3:
            st.header(" ")
            try:
                cash_value=networth["cash"].values[-1]
                cash_value2=networth["cash"].values[-2]
                change_last_month=(networth["cash"].values[-1]-networth["cash"].values[-2])
                fcash_value=f"${cash_value:,.2f}"
                fchange_last_month=f"{change_last_month:,.2f}"
                logger.debug(f"Cash value: {fcash_value}, Change: {fchange_last_month}")
                st.metric(label="Cash", value=fcash_value, delta=fchange_last_month, delta_color="normal")
            except (IndexError, KeyError) as e:
                logger.error(f"Error calculating cash metrics: {str(e)}", exc_info=True)
                st.error("Unable to load cash data")
                
        with col1row4:
            st.header(" ")
            try:
                taxable_value=networth["taxable"].values[-1]
                taxable_last_month=networth["taxable"].values[-1]-networth["taxable"].values[-2]
                ftaxable_value=f"${taxable_value:,.2f}"
                ftaxable_last_month=f"{taxable_last_month:,.2f}"
                logger.debug(f"Taxable value: {ftaxable_value}, Change: {ftaxable_last_month}")
                st.metric(label="Brokerage", value=ftaxable_value, delta=ftaxable_last_month, delta_color="normal")
            except (IndexError, KeyError) as e:
                logger.error(f"Error calculating taxable metrics: {str(e)}", exc_info=True)
                st.error("Unable to load brokerage data")
                
        with col1row5:
            st.header(" ")
            try:
                roth_value=networth["tax_free"].values[-1]
                roth_last_month=networth["tax_free"].values[-1]-networth["tax_free"].values[-2]
                froth_value=f"${roth_value:,.2f}"
                froth_last_month=f"{roth_last_month:,.2f}"
                logger.debug(f"Roth value: {froth_value}, Change: {froth_last_month}")
                st.metric(label="Roth", value=froth_value, delta=froth_last_month, delta_color="normal")
            except (IndexError, KeyError) as e:
                logger.error(f"Error calculating Roth metrics: {str(e)}", exc_info=True)
                st.error("Unable to load Roth data")
                
        with col1row6:
            st.header(" ")
            try:
                trad_value=networth["tax_deferred"].values[-1]
                trad_last_month=networth["tax_deferred"].values[-1]-networth["tax_deferred"].values[-2]
                ftrad_value=f"${trad_value:,.2f}"
                ftrad_last_month=f"{trad_last_month:,.2f}"
                logger.debug(f"Traditional value: {ftrad_value}, Change: {ftrad_last_month}")
                st.metric(label="Traditional", value=ftrad_value, delta=ftrad_last_month, delta_color="normal")
            except (IndexError, KeyError) as e:
                logger.error(f"Error calculating Traditional metrics: {str(e)}", exc_info=True)
                st.error("Unable to load Traditional IRA data")
                
        with col1row7:
            st.header(" ")
            try:
                total_value=networth["total"].values[-1]
                total_last_month=networth["total"].values[-1]-networth["total"].values[-2]
                ftotal_value=f"${total_value:,.2f}"
                ftotal_last_month=f"{total_last_month:,.2f}"
                logger.debug(f"Total net worth: {ftotal_value}, Change: {ftotal_last_month}")
                st.metric(label="Total Net Worth", value=ftotal_value, delta=ftotal_last_month, delta_color="normal")
            except (IndexError, KeyError) as e:
                logger.error(f"Error calculating total net worth metrics: {str(e)}", exc_info=True)
                st.error("Unable to load total net worth data")
                
    except Exception as e:
        logger.error(f"Error rendering net worth metrics section: {str(e)}", exc_info=True)
        st.error(f"Error displaying net worth metrics: {str(e)}")



    if "visibility" not in st.session_state:
       st.session_state.visibility = "visible"
       st.session_state.disabled = False
  
    with st.expander("Create estimated taxes for next year"):
        col5, col6, col7, col8, col14 = st.columns(5)    
        with col5: 
            wages = st.number_input("Wages",  on_change=clear_submit)
            # wages = st.number_input("Wages",step=None)
        with col6:     
            deferred_distribution = st.number_input("Trad IRA Distribution", on_change=clear_submit)
        with col14:    
            interest=st.number_input("Interest", on_change=clear_submit)
            #print(f"Input interest is {interest:.6f}")
        with col7:     
            cg_income_lt = st.number_input("Long Term Cap Gains", on_change=clear_submit)
            #print(f"Input LTCG is {cg_income_lt:.6f}")
        with col8:
            cg_income_st = st.number_input("Short Term Cap Gains", on_change=clear_submit)
            #print(f"Input STCG is {cg_income_st:.6f}")
      
        col9, col10, col12, col11, col13 = st.columns(5)  
        with col9:
           people = st.selectbox("Medicare Eligible" ,[0,1,2], on_change=clear_submit)
        with col10:
           year =st.selectbox("Tax Year" ,[2023,2024,2025,2026,2027], on_change=clear_submit,index=3)
        with col12:    
           maxdaf=st.selectbox("Max Donor Advisor Fund" ,['N','Y'], on_change=clear_submit)
        with col11:
          if maxdaf == 'Y':
             daf1 =st.number_input("Charitable Contrib", disabled=True )
          else:   
             daf1 =st.number_input("Charitable Contrib", on_change=clear_submit)
        with col13:
             headroom_rate = (st.selectbox("Max Conversion Rate" ,[10,12,22,24,32,35,37], on_change=clear_submit,index=3)/100)
        col14,col15,col16,col17,col18= st.columns(5) 
        with col14:
             roth_amount = st.number_input("Roth Conversion Amount" , on_change=clear_submit)
        with col15:    
             pd_tax_amount = st.number_input("Estimated prepaid Fed taxes" , on_change=clear_submit)
        summarize_button = st.button("Project this years changes!")
   

    if summarize_button:
        try:
            logger.debug(f"Starting tax calculation for year {year}")
            
            # Load tax data with exception handling
            try:
                taxratedf = get_income_tax_brackets(year)
                cgdf = get_cap_gains_brackets(year)
                irmaadf= get_medicare_costs(year)
                stddectdf = get_std_deduction(year)
                atmdf = get_atm_costs(year)
                logger.debug("Tax data loaded successfully")
            except Exception as e:
                logger.error(f"Error loading tax data: {str(e)}", exc_info=True)
                st.error(f"Error loading tax data for year {year}: {str(e)}")
                st.stop()
        
            # Calculate DAF with exception handling
            try:
                logger.debug(f"Calculating DAF: deferred={deferred_distribution}, wages={wages}, interest={interest}, daf1={daf1}, maxdaf={maxdaf}")
                calc_daf = calc_daf_value(deferred_distribution+wages,interest,daf1,maxdaf)
                logger.debug(f"Calculated DAF: {calc_daf}")
            except Exception as e:
                logger.error(f"Error calculating DAF: {str(e)}", exc_info=True)
                st.error(f"Error calculating Donor Advisor Fund: {str(e)}")
                calc_daf = 0

            # Calculate AGI with exception handling
            try:
                logger.debug(f"Calculating AGI: income={deferred_distribution+wages+cg_income_st}, interest={interest}")
                agi = calc_agi(deferred_distribution+wages+cg_income_st,interest,stddectdf,calc_daf)
                logger.debug(f"Calculated AGI: {agi}")
            except Exception as e:
                logger.error(f"Error calculating AGI: {str(e)}", exc_info=True)
                st.error(f"Error calculating Adjusted Gross Income: {str(e)}")
                st.stop()
       
            # Calculate IRMAA fees with exception handling
            try:
                irmaa_fees_income = calculate_irmma_penalty(agi, irmaadf, people)
                logger.debug(f"Calculated IRMAA fees: {irmaa_fees_income}")
            except Exception as e:
                logger.error(f"Error calculating IRMAA penalty: {str(e)}", exc_info=True)
                st.error(f"Error calculating Medicare surcharge: {str(e)}")
                irmaa_fees_income = 0
       
            # Calculate taxable income with exception handling
            try:
                logger.debug(f"Calculating taxable income for AGI: {agi}")
                taxable_income, maxrate, uppermax = calculate_taxable_income(agi, taxratedf)
                logger.debug(f"Taxable income: {taxable_income}, Max rate: {maxrate}, Upper max: {uppermax}")
            except Exception as e:
                logger.error(f"Error calculating taxable income: {str(e)}", exc_info=True)
                st.error(f"Error calculating taxable income: {str(e)}")
                st.stop()
       
            # Calculate headroom with exception handling
            try:
                headroom_max = getUpperIncomeRate(headroom_rate,taxratedf)
                logger.debug(f"Headroom max: {headroom_max}")
            except Exception as e:
                logger.error(f"Error calculating headroom: {str(e)}", exc_info=True)
                st.error(f"Error calculating conversion headroom: {str(e)}")
                headroom_max = 0
       
            if maxrate>headroom_rate:
                logger.warning(f"Max rate {maxrate} exceeds headroom rate {headroom_rate}")
                st.warning("Current tax rate exceeds target conversion rate")
                
            lowerby=0
            
            # Calculate ATM values with exception handling
            try:
                atm_lower,atm_deduction = getlower_atm_amount_n_deduction(year, atmdf)
                logger.debug(f"ATM Values: lower={atm_lower}, deduction={atm_deduction}")
                std_deduction=get_std_deduction_by_year(year)
                if uppermax>=(atm_lower+atm_deduction):
                    uppermax=atm_lower+atm_deduction
                    logger.debug(f"Adjusted uppermax to ATM threshold: {uppermax}")
            except Exception as e:
                logger.error(f"Error calculating ATM values: {str(e)}", exc_info=True)
                st.error(f"Error calculating Alternative Minimum Tax values: {str(e)}")
                atm_lower, atm_deduction = 0, 0
                
            # Calculate Roth conversions with exception handling
            try:
                if roth_amount>0:
                    conversions=roth_amount
                    logger.debug(f"Using specified Roth conversion amount: {conversions}")
                    conversion_tax= calc_roth_conversions_tax(maxrate,headroom_rate,uppermax,agi,headroom_max,conversions)
                    logger.debug(f"Conversion tax: {conversion_tax}")
                else:
                    logger.debug(f"Calculating optimal Roth conversion for AGI: {agi}")
                    conversions,conversion_tax = calc_roth_conversions(maxrate,headroom_rate,uppermax,agi,headroom_max,lowerby)
                    logger.debug(f"Optimal conversion: {conversions}, tax: {conversion_tax}")
            except Exception as e:
                logger.error(f"Error calculating Roth conversions: {str(e)}", exc_info=True)
                st.error(f"Error calculating Roth conversion: {str(e)}")
                conversions, conversion_tax = 0, 0
        
            # Recalculate AGI with conversions
            try:
                if conversions >= 0:
                    agi = calc_agi(deferred_distribution+wages+cg_income_st+conversions,interest,stddectdf,calc_daf)
                    taxable_income, maxrate, uppermax = calculate_taxable_income(agi, taxratedf)
                    logger.debug(f"Recalculated AGI with conversions: {agi}")
            except Exception as e:
                logger.error(f"Error recalculating AGI with conversions: {str(e)}", exc_info=True)
                st.error(f"Error recalculating income with Roth conversion: {str(e)}")
     
            # Calculate capital gains tax with exception handling
            try:
                if cg_income_lt == 0:
                    cg_tax=0
                else:
                    cg_tax = calculate_cap_gains(agi, cgdf, cg_income_lt)
                    logger.debug(f"Capital gains tax: {cg_tax}")
            except Exception as e:
                logger.error(f"Error calculating capital gains tax: {str(e)}", exc_info=True)
                st.error(f"Error calculating capital gains tax: {str(e)}")
                cg_tax = 0
            
            # Check ATM threshold
            if agi >=(atm_lower+atm_deduction+calc_daf+30000):
                logger.warning(f"MAGI {agi+conversions+cg_income_lt} exceeds ATM threshold {atm_lower+atm_deduction+calc_daf+30000}")
        
            # Calculate ATM tax with exception handling
            try:
                atm_tax,init_lowerby = calculate_atm(agi,cg_income_lt, atmdf)
                logger.debug(f"ATM tax: {atm_tax}, taxable income: {taxable_income}")
                
                if taxable_income>atm_tax:
                    atm_tax=0
                    logger.debug(f"ATM tax set to 0 (taxable income {taxable_income} > ATM tax)")
                else:
                    logger.debug(f"ATM tax applies: MAGI={agi+cg_income_lt}")
            except Exception as e:
                logger.error(f"Error calculating ATM tax: {str(e)}", exc_info=True)
                st.error(f"Error calculating Alternative Minimum Tax: {str(e)}")
                atm_tax, init_lowerby = 0, 0
        
            # Calculate IRMAA with headroom
            try:
                irmaa_fees_income_headroom = calculate_irmma_penalty(uppermax, irmaadf, people)
                logger.debug(f"IRMAA fees with headroom: {irmaa_fees_income_headroom}")
            except Exception as e:
                logger.error(f"Error calculating IRMAA with headroom: {str(e)}", exc_info=True)
                st.error(f"Error calculating Medicare surcharge with conversion: {str(e)}")
                irmaa_fees_income_headroom = 0
                
        except Exception as e:
            logger.error(f"Unexpected error in tax calculation: {str(e)}", exc_info=True)
            st.error(f"An unexpected error occurred during tax calculation: {str(e)}")
            st.stop()
   
        # Display results with exception handling
        try:
            logger.debug("Rendering calculation results")
            col1row3, col1row5, col1row6,col1row7,col1row8 = st.columns(5)
            with col1row3:
                st.markdown('##### Ordinary Income')
                fcash_value=f"${agi:,.2f}"
                f_cgst_month=f"${cg_income_st:,.2f}"
                fcg_taxable_value=f"${cg_income_lt:,.2f}"
                fint_value=f"${interest:,.2f}"
                if 0 < (agi+conversions):
                    st.metric(label="Adjusted Gross Income", value=fcash_value)
                if 0 < (interest):
                    st.metric(label="Interest", value=fint_value)
                if 0 < (cg_income_st):
                    logger.debug(f"Displaying STCG: {cg_income_st:.6f}")
                    st.metric(label="Short Term Capital Gains", value=f_cgst_month)
                if 0 != (cg_income_lt):
                    st.metric(label="Long Term Capital Gains", value=fcg_taxable_value)

            with col1row5:
                st.markdown('##### Taxes Owed')
                fincome_tax=f"${taxable_income+cg_tax-pd_tax_amount:,.2f}"
                fcg_tax=f"${cg_tax:,.2f}"
                froth_conv_tax=f"${conversion_tax:,.2f}"
                
                # Calculate state tax
                if 0 == (wages+cg_income_lt+cg_income_st+interest):
                    state_tax=0
                    quarterly_state_tax=0
                else:
                    state_tax=((wages+cg_income_lt+cg_income_st+interest)*0.03 )
                    quarterly_state_tax=state_tax/4
                    
                fstate_tax=f"${state_tax:,.2f}"
                fquarterly_state_tax=f"${quarterly_state_tax:,.2f}"
                quarterly_fed_tax=(taxable_income+cg_tax-pd_tax_amount)/4
                fquarterly_fed_tax=f"${quarterly_fed_tax:,.2f}"
                
                if taxable_income>0:
                    st.metric(label="Income Tax", value=fincome_tax)
                    st.metric(label="Quarterly Fed tax Payment", value=fquarterly_fed_tax)
                if state_tax>0:
                    st.metric(label="State tax", value=fstate_tax)
                    st.metric(label="Quarterly State tax Payment", value=fquarterly_state_tax)
           
         
                st.markdown('##### Other costs')
                fdaf=f"${calc_daf:,.2f}"
                fdeferred_distribution=f"${deferred_distribution:,.2f}"
                firmaa_fees=f"${irmaa_fees_income:,.2f}"
                firmaa_headroom_month=f"${irmaa_fees_income_headroom:,.2f}"
                ftaxable_last_month=f"${cg_income_st:,.2f}"
                fatm_tax_taxable_income=f"${atm_tax-taxable_income:,.2f}"
                flowerby=f"${lowerby:,.2f}"
                
                if calc_daf>0:
                    st.metric(label="Donor Advisory Fund", value=fdaf)
                if cg_tax>0:
                    st.metric(label="Long Term Capital Gains Tax", value=fcg_tax)
                if  irmaa_fees_income >0:
                    st.metric(label="Medicare Surcharge", value=firmaa_fees)
                if irmaa_fees_income_headroom>0 and conversions>0:
                    st.metric(label="Medicare cost w. Roth Conversion", value=firmaa_headroom_month)
                if atm_tax>taxable_income:
                    st.metric(label="Additional ATM taxes", value=fatm_tax_taxable_income)
                    st.metric(label="Decrease income or LT Capital gains by", value=flowerby)
                    
            with col1row6:
                st.markdown('##### Traditional Updates')
                ftrad_value=f"${trad_value:,.2f}"
                ftrad_dist_delta=f"${trad_value-deferred_distribution-conversions:,.2f}"
                fdelta=f"{-deferred_distribution-conversions:,.2f}"
                if deferred_distribution+conversions >0:
                    st.metric(label="New Traditional Balance", value=ftrad_dist_delta, delta=fdelta)
                else:
                    st.metric(label="Pre Changes Traditional", value=ftrad_value)
                    
            with col1row7:
                st.markdown('##### Roth Updates')
                fconversions_amt=f"{conversions:,.2f}"
                fconversions=f"${conversions:,.2f}"
                fconversions_total=f"${(roth_value+conversions):,.2f}"
                if conversions > 0:
                    st.metric(label="New Roth Account Balance", value=fconversions_total, delta=fconversions_amt)
                    st.metric(label="Roth Conversion",value=fconversions)
                    st.metric(label="Estimated Roth Conversion tax", value=froth_conv_tax)
                else:
                    st.metric(label="Pre Changes Roth", value=froth_value)
                    
            with col1row8:
                st.markdown('##### Broker & Cash Updates')
                fcash_value=f"${cash_value:,.2f}"
                new_cash_value=(cash_value-state_tax-cg_tax-taxable_income+pd_tax_amount)
                new_broker_value =(taxable_value-calc_daf+deferred_distribution)
                fnew_cash_delta=f"{new_cash_value-cash_value:,.2f}"
                f_new_broker_value=f"${new_broker_value:,.2f}"
                f_new_broker_delta_value=f"{new_broker_value-taxable_value:,.2f}"
                f_new_cash_value=f"${new_cash_value:,.2f}"
                
                if new_cash_value != cash_value:
                    st.metric(label="New Cash Balance", value=f_new_cash_value, delta=fnew_cash_delta)
                else:
                    st.metric(label="Pre Changes Cash Balance", value=fcash_value)
                if new_broker_value != taxable_value:
                    st.metric(label="New Broker Balance", value=f_new_broker_value, delta=f_new_broker_delta_value)
                else:
                    st.metric(label="Pre Changes Broker Balance", value=ftaxable_value)
                    
            logger.debug("Results rendered successfully")
            
        except Exception as e:
            logger.error(f"Error rendering calculation results: {str(e)}", exc_info=True)
            st.error(f"Error displaying calculation results: {str(e)}")
with something_tab:
    st.header("something calculator")